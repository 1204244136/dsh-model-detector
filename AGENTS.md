...
0. **线上声明**（`DeclaredModel`）——**端点自己在 `/models` 里声明的能力值，优先级最高**。`fetchLiveModels()` 逐条解析 `context_length`/`context_window`/`max_allowed_size`、`max_output_tokens`/`max_completion_tokens`、`supports_images`/`vision`（**显式的 `false` 也算声明**："本端点不收图"）、`supports_reasoning`、`reasoning_supported_efforts`、`name`，以及**纯展示用**的 `credits`（计费倍率，归一成 `x<数字>`：实测同一端点混用 `"x0.21"` 与 `"x0.34 credits"` 两种写法）。为什么压过 models.dev：这是端点对自己能力的陈述，而 models.dev 是第三方快照（可能滞后，也可能抄的是别家网关的乐观值——实测 `cn:deepseek-v4-flash` 被 models.dev 写成 `maxTokens=384000`，端点自己声明 `max_output_tokens: 50000`，照 384000 发请求会被上游拒）。WorkBuddy 的 65 条全靠它才 100% 命中（含 models.dev 根本没有的 `hy4-preview-f`/`hy3-x`/`cn:auto`/`-volc`/`-lkeap` 变体）。**逐字段生效**：端点没声明的字段仍按下面的顺序回落；只声明 `id/object/created/owned_by` 的路由（如 antigravity）行为完全不变。来源标注为 `provider`（UI 显示「线上声明」）。
   ...
   - **`credits` 是展示字段**：只出现在发现页的 chip（`x0.00` 显示为绿色「免费」样式），**不得写进配置**——`cleanForTarget` 的白名单会丢弃它，`verify.mjs` 有专门用例守着这条（DSH 的 modelProfile / catalogModel 都不认这个字段）。
